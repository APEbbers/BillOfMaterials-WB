...
class General_BOM:
    ...
    def createBoMSpreadsheet(self, mainList: list, Headers: dict = None, Summary: bool = False):
        ...
        for i in range(len(CopyMainList)):
            ...
            sheet.set(
                "D" + str(Row), self.ReturnDocProperty(rowList["DocumentObject"], "Label2")
            ...
            for i in range(4, len(Headers) + 1):
                ...
                if Headers[Column + "1"] == "Type":
                    ...
                    sheet.set(Column + str(Row), self.ReturnDocProperty(rowList["DocumentObject"], "Label"))
                elif Headers[Column + "1"].lower() == "name":
                    sheet.set(Column + str(Row), self.ReturnDocProperty(rowList["DocumentObject"], "Name"))
                elif Headers[Column + "1"].lower() == "fullname":
                    sheet.set(Column + str(Row), self.ReturnDocProperty(rowList["DocumentObject"], "FullName"))
                elif Headers[Column + "1"].lower() == "typeid":
                    sheet.set(Column + str(Row), self.ReturnDocProperty(rowList["DocumentObject"], "TypeId"))
                else:
                    sheet.set(
                        Column + str(Row), self.ReturnViewProperty(rowList["DocumentObject"], Headers[Column + "1"])
                    )
        ...
        if Row > 1:
            sheet.setAlignment(
                "A1:" + str(Standard_Functions.GetLetterFromNumber(len(Headers))) + str(Row), "center", "keep"
            )
        ...
        sheet.set("A" + str(Row + 2), f"{translate('BoM Workbench', 'BoM created by')}:   {CreatedBy}")
        sheet.set(
            "A" + str(Row + 3),
